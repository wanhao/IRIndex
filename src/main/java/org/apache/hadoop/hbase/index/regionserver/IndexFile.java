...
import org.apache.hadoop.hbase.index.regionserver.IndexKeyValue.IndexKeyComparator;
...
public class IndexFile {
  static final Log LOG = LogFactory.getLog(IndexFile.class);
  ...
  public static class Writer implements Closeable {
     ...
     * @throws IOException
     */
    public Writer(FileSystem fs, Path path, short replication)
    throws IOException {
      this(fs, path, replication, DEFAULT_BLOCKSIZE, (Compression.Algorithm) null, null);
    }
    ...
    public Writer(final FSDataOutputStream ostream, final int blocksize,
      final String  compress, final IndexKeyComparator c)
    throws IOException {
     ...
     */
    public Writer(final FSDataOutputStream ostream, final int blocksize,
      final Compression.Algorithm  compress, final IndexKeyComparator c)
    throws IOException {
      this.outputStream = ostream;
      this.closeOutputStream = false;
      this.blocksize = blocksize;
      this.comparator = c == null? IndexKeyValue.COMPARATOR.getRawComparator(): c;
     ...
     * on construction.
     * @param kv IndexKeyValue to add.  Cannot be empty nor null.
     * @throws IOException
     */
    public void append(final IndexKeyValue kv)
    throws IOException {
      ...
      if (this.lastKeyBuffer != null) {
        ...
        if (keyComp > 0) {
          throw new IOException("Added a key not lexically larger than" +
            " current key=" + Bytes.toStringBinary(key, offset, length) +
  ...
  public static class Reader implements Closeable {
    ...
    IndexFile.BlockIndex blockIndex;
    ...
    protected String toStringFirstKey() {
      return IndexKeyValue.keyToString(getFirstKey());
    }

    protected String toStringLastKey() {
      return IndexKeyValue.keyToString(getLastKey());
    }
     ...
     * Call {@link IndexFileScanner#seekTo(byte[])} to position an start the read.
     ...
     */
    public IndexFileScanner getScanner(boolean cacheBlocks, final boolean pread) {
      ...
      try {
        ...
        InputStream is = this.compressAlgo.createDecompressionStream(
            new BufferedInputStream(
                new BoundedRangeIndexFileInputStream(this.istream, offset, compressedSize,
                                                pread),
     ...
     * to eliminate IndexKeyValue here
     */
    public byte[] getFirstRowKey() {
      byte[] firstKey = getFirstKey();
      if (firstKey == null) return null;
      return IndexKeyValue.createIndexKeyValueFromKey(firstKey).getRow();
    }
    ...
    public byte[] getLastRowKey() {
      ...
      return IndexKeyValue.createIndexKeyValueFromKey(lastKey).getRow();
    }
    ...
    public long[] getRangeScale(Range range) {
      IndexKeyValue ikv = null;
      long totalSize = 0;
      long startBlockOffset = 0, stopBlockOffset = this.trailer.fileinfoOffset;

      ikv = IndexKeyValue.createFirstOnQualifier(range.getQualifier(), new byte[] { (byte) 0x00 });
      int startblockid =
          this.blockContainingKey(ikv.getBuffer(), ikv.getKeyOffset(), ikv.getKeyLength());
      if (startblockid >= 0) {
        startBlockOffset = this.blockIndex.blockOffsets[startblockid];
      }
      ikv = IndexKeyValue.createFirstOnQualifier(range.getQualifier(), new byte[] { (byte) 0xff });
      int stopblockid =
          this.blockContainingKey(ikv.getBuffer(), ikv.getKeyOffset(), ikv.getKeyLength());
      if (stopblockid != this.trailer.dataIndexCount - 1) {
        stopBlockOffset = this.blockIndex.blockOffsets[stopblockid+1];
      }
      totalSize = stopBlockOffset - startBlockOffset;
      ...
      if (range.getStartValue() != null) {
        ...
        case EQUAL:
          ikv = IndexKeyValue.createFirstOnQualifier(range.getQualifier(), range.getStartValue());
          blockid =
              this.blockContainingKey(ikv.getBuffer(), ikv.getKeyOffset(), ikv.getKeyLength());
          if (blockid == -1) {
            stopBlockOffset = 0;
          ...
          startBlockOffset = this.blockIndex.blockOffsets[blockid];
          ikv = IndexKeyValue.createLastOnQualifier(range.getQualifier(), range.getStartValue());
          blockid =
              this.blockContainingKey(ikv.getBuffer(), ikv.getKeyOffset(), ikv.getKeyLength());
          if (blockid != this.trailer.dataIndexCount - 1) {
            stopBlockOffset = this.blockIndex.blockOffsets[blockid + 1];
          }
          break;
        case GREATER:
          ikv = IndexKeyValue.createLastOnQualifier(range.getQualifier(), range.getStartValue());
      ...
      if (range.getStopValue() != null) {
        ...
        case LESS:
          ikv = IndexKeyValue.createFirstOnQualifier(range.getQualifier(), range.getStopValue());
          blockid =
              this.blockContainingKey(ikv.getBuffer(), ikv.getKeyOffset(), ikv.getKeyLength());
          if (blockid == -1) {
            stopBlockOffset = 0;
          } else if (blockid != this.trailer.dataIndexCount - 1) {
            stopBlockOffset = this.blockIndex.blockOffsets[blockid + 1];
          }
          break;
        case LESS_OR_EQUAL:
          ikv = IndexKeyValue.createLastOnQualifier(range.getQualifier(), range.getStopValue());
          blockid =
              this.blockContainingKey(ikv.getBuffer(), ikv.getKeyOffset(), ikv.getKeyLength());
          if (blockid == -1) {
            stopBlockOffset = 0;
      ...
      rangeSize = stopBlockOffset - startBlockOffset;
      
      if (totalSize < 0 || rangeSize < 0) throw new RuntimeException("Invalid Range:" + range);

      return new long[] { rangeSize, totalSize };
    }

    /*
     * Implementation of {@link HFileScanner} interface.
     */
    protected static class Scanner implements IndexFileScanner {
      ...
      public IndexKeyValue getIndexKeyValue() {
        ...
        return new IndexKeyValue(this.block.array(),
  ...
  static class BlockIndex implements HeapSize {
    ...
    public String toString() {
      ...
      for (int i = 0; i < count ; i++) {
        sb.append("key=").append(IndexKeyValue.keyToString(blockKeys[i])).
  ...
  public static String[] getSupportedCompressionAlgorithms() {
    return HFile.getSupportedCompressionAlgorithms();
  }
  ...
  public static void main(String []args) throws IOException {
    try {
      ...
      options.addOption("s", "scale", false, "print range scale");
      ...
      boolean printScale = cmd.hasOption("s");
      ...
      for (Path file : files) {
        ...
        IndexFile.Reader reader = new IndexFile.Reader(fs, file, null, false);
        Map<byte[],byte[]> fileInfo = reader.loadFileInfo();
        int count = 0;
        if (verbose || printKey || checkRow || checkFamily) {
          // scan over file and read key/value's and check if requested
          IndexFileScanner scanner = reader.getScanner(false, false);
          scanner.seekTo();
          IndexKeyValue pkv = null;
          do {
            IndexKeyValue kv = scanner.getIndexKeyValue();
            // dump key value
           if (printKey) {
             System.out.print("K: " + Bytes.toStringBinary(kv.getQualifier())+","+
            		 Bytes.toStringBinary(kv.getQualifierValue()));
             if (printValue) {
...
//          tmp = reader.getRangeScale(new Range(Bytes.toBytes("f:c3"), null, CompareOp.NO_OP, Bytes
//                  .toBytes(30000.0), CompareOp.LESS));
//          System.out.println(tmp[0] + "," + tmp[1] + "," + (1.0 * tmp[0] / tmp[1]));
//
//          tmp =
//              reader.getRangeScale(new Range(Bytes.toBytes("f:c3"), Bytes.toBytes(30000.0),
//                  CompareOp.GREATER, null, CompareOp.NO_OP));
//          System.out.println(tmp[0] + "," + tmp[1] + "," + (1.0 * tmp[0] / tmp[1]));
//          
//          tmp =
//              reader.getRangeScale(new Range(Bytes.toBytes("f:c3"), Bytes.toBytes(30000.0),
...
//
//          tmp =
//              reader.getRangeScale(new Range(Bytes.toBytes("f:c5"), Bytes.toBytes("2-HIGH"),
//                  CompareOp.EQUAL, null, CompareOp.NO_OP));
//          System.out.println(tmp[0] + "," + tmp[1] + "," + (1.0 * tmp[0] / tmp[1]));
          ...
          tmp =
              reader.getRangeScale(new Range(Bytes.toBytes("f:c5"), Bytes.toBytes("2-HIGH"),
                  CompareOp.GREATER_OR_EQUAL, null, CompareOp.NO_OP));
          System.out.println(tmp[0] + "," + tmp[1] + "," + (1.0 * tmp[0] / tmp[1]));

          tmp =
              reader.getRangeScale(new Range(Bytes.toBytes("f:c5"), null, CompareOp.NO_OP, Bytes
                  .toBytes("2-HIGH"), CompareOp.LESS));
          System.out.println(tmp[0] + "," + tmp[1] + "," + (1.0 * tmp[0] / tmp[1]));

          tmp =
              reader.getRangeScale(new Range(Bytes.toBytes("f:c5"), null, CompareOp.NO_OP, Bytes
                  .toBytes("2-HIGH"), CompareOp.LESS_OR_EQUAL));
          System.out.println(tmp[0] + "," + tmp[1] + "," + (1.0 * tmp[0] / tmp[1]));
        }
